function [c,dc] = robustVariancecost_check_xonly_ii(obj, x, X0, ii)

x_full = X0(1:obj.nx*obj.N);
x_full((ii-1)*12+1:ii*12) = x;%X0(1:obj.nx*obj.N);
...
for k = 1:obj.N-1%[Ye: double check the index]
    ...
    for j = 1:(2*(obj.nx+nw))
        ...
        %                     [phi,normal,~,~,~,~,~,~,n,D,dn,dD] = obj.plant.contactConstraints(Sig(1:obj.nx/2,j),false,obj.objions.active_collision_objions);
        %                     J = zeros(nl,nq);
        %                     J(1:1+obj.nD:end,:) = n;
        %                     dJ = zeros(nl*nq,nq);
