classdef DirectTrajectoryOptimization_Brick < NonlinearProgram
  %DIRECTTRAJECTORYOPTIMIZATION_BRICK An abstract class for direct method approaches to
  % trajectory optimization for falling brick example.
  %
  ...
  methods
    function obj = DirectTrajectoryOptimization_Brick(plant,N,durations,options)
    % function obj =
    % DirectTrajectoryOptimization_Brick(plant,initial_cost,running_cost,final_cost,...
    ...
    %#ok<*PROP>
      ...
      if ~plant.isTI
        error('Drake:DirectTrajectoryOptimization_Brick:UnsupportedPlant','Only time-invariant plants are currently supported');
      end
    ...
    function obj = addTrajectoryDisplayFunction(obj,display_fun)
      ...
      obj = addDisplayFunction(obj,@(z) display_fun(z(obj.h_inds),z(obj.x_inds),z(obj.u_inds),z(obj.F_ext_inds)));%,z(obj.LCP_slack_inds)
    end
