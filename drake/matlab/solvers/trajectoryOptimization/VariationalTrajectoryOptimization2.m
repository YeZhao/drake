classdef VariationalTrajectoryOptimization2 < DirectTrajectoryOptimization
    ...
    methods
        function obj = VariationalTrajectoryOptimization2(plant,N,duration,options)
            if nargin < 4
                options=struct(); 
            end
        ...
        function obj = setupVariables(obj, N)
            switch obj.options.integration_method
                case VariationalTrajectoryOptimization.MIDPOINT
                    ...
                    num_vars = nH + N*nX + (N-1)*nU + (N-1)*nL;
                    
                    obj.h_inds = (1:nH)';
                    obj.x_inds = reshape(nH + (1:(N*nX)), nX, N);
                    obj.u_inds = reshape(nH + N*nX + (1:((N-1)*nU)), nU, N-1);
                    obj.psi_inds = reshape(nH + N*nX+(N-1)*nU + (1:((N-1)*nC)), nC, N-1);
                    obj.eta_inds = reshape(nH + N*nX + (N-1)*nU + (N-1)*nC + (1:((N-1)*nD*nC)), nD*nC, N-1);
                    obj.c_inds = reshape(nH + N*nX + (N-1)*nU + (N-1)*nC + (N-1)*nD*nC + (1:((N-1)*nC)), nC, N-1);
                    obj.b_inds = reshape(nH + N*nX + (N-1)*nU + 2*(N-1)*nC + (N-1)*nD*nC + (1:((N-1)*nC*nD)), nD*nC, N-1);
                    obj.s_inds = nH + N*nX + (N-1)*nU + 2*(N-1)*nC + 2*(N-1)*nD*nC + (1:(N-1))';
                    ...
                    for i = 1:N
                        ...
                        if(i<N)
                            ...
                            for j = 1:nU
                                x_names{nH + N*nX + (i-1)*nU+j} = sprintf('u%d[%d]',j,i);
                            end
                            for j = 1:nC
                                x_names{nH + N*nX + (N-1)*nU + (i-1)*nC+j} = sprintf('psi%d[%d]',j,i);
                                x_names{nH + N*nX + (N-1)*nU + (N-1)*nC + (N-1)*nC*nD + (i-1)*nC+j} = sprintf('c%d[%d]',j,i);
                            end
                            for j = 1:(nC*nD)
                                x_names{nH + N*nX + (N-1)*nU + (N-1)*nC + (i-1)*nC*nD+j} = sprintf('eta%d[%d]',j,i);
                                x_names{nH + N*nX + (N-1)*nU + 2*(N-1)*nC + (N-1)*nC*nD + (i-1)*nC*nD+j} = sprintf('b%d[%d]',j,i);
                            end
                            x_names{nH + N*nX + (N-1)*nU + 2*(N-1)*nC + 2*(N-1)*nC*nD + i} = sprintf('s[%d]',i);
                        end
                    end
...
%                     cnstr_opts.diff_type = 'central';
                    ...
                    nvars1 = 2+3*nX+2*nU+nC+nD*nC;
                    cnstr1 = FunctionHandleConstraint(zeros(3*nQ,1), zeros(3*nQ,1), nvars1, @obj.midpoint_dynamics_constraint_fun, cnstr_opts);
                    
                    if nC
                        nvars2 = 1+nX+2*nC+2*nD*nC+1;
                        ...
                        for i = 1:obj.N-1
                            cont_inds{i} = {obj.h_inds(i); obj.x_inds(:,i+1); obj.psi_inds(:,i); obj.eta_inds(:,i); obj.c_inds(:,i); obj.b_inds(:,i); obj.s_inds(i)};
                    ...
                    dyn_inds{1} = {obj.h_inds(1); obj.x_inds(:,1); obj.x_inds(:,2); obj.u_inds(:,1); obj.c_inds(:,1); obj.b_inds(:,1)};
                    dyn_constraints{1} = FunctionHandleConstraint(zeros(nQ,1), zeros(nQ,1), 1+2*nX+nU+nC+nD*nC, @obj.midpoint_first_step_fun, cnstr_opts);
                    dyn_constraints{1} = dyn_constraints{1}.setName('dynamics[1]');
        ...
        function obj = addPositionConstraint(obj,constraint,time_index,q_indices)
            if ~iscell(time_index)
                % then use { time_index(1), time_index(2), ... } ,
                % aka independent constraints for each time
                time_index = num2cell(reshape(time_index,1,[]));
            ...
            if nargin < 4
                q_indices = 1:(size(obj.x_inds,1)/2);
            end
            
            for j=1:length(time_index)
                
                cstr_inds = mat2cell(obj.x_inds(q_indices,time_index{j}),numel(q_indices),ones(1,length(time_index{j})));
        ...
        function obj = addVelocityConstraint(obj,constraint,time_index,v_indices)
            nQ = obj.plant.getNumPositions();
            if ~iscell(time_index)
                % then use { time_index(1), time_index(2), ... } ,
                % aka independent constraints for each time
        ...
        function obj = addRunningCost(obj,running_cost_function)
            ...
            switch obj.integration_method
                case VariationalTrajectoryOptimization.MIDPOINT
                    for i = 1:obj.N-1
                        running_cost = FunctionHandleObjective(1+2*nX+nU, @(h,x1,x2,u)midpoint_running_fun(obj,running_cost_function,h,x1,x2,u));
        ...
        function obj = addFinalCost(obj,final_cost_function)
            ...
            switch obj.integration_method
                case VariationalTrajectoryOptimization.MIDPOINT
                    final_cost = FunctionHandleObjective(2+nX, @(h,xf)midpoint_final_fun(obj,final_cost_function,h,xf));
                    inds_i = {obj.h_inds(:); obj.x_inds(:,end)};
        ...
        function [f,df] = midpoint_first_step_fun(obj,h,x0,x1,u,c,b)
...
%             disp(max(abs(df_fd(:)-df(:))));
        ...
        function [f,df] = midpoint_first_step(obj,xin)
            ...
            x1 = xin(1+nX+(1:nX));
            u = xin(1+2*nX+(1:nU));
            c = xin(1+2*nX+nU+(1:nC));
            b = xin(1+2*nX+nU+nC+(1:nD*nC));
        ...
        function [f,df] = midpoint_dynamics_constraint_fun(obj,h1,h2,x1,x2,x3,u1,u2,c2,b2)
            
            xin = [h1;h2;x1;x2;x3;u1;u2;c2;b2];
...
%             disp(max(abs(df_fd(:)-df(:))));
        ...
        function [f,df] = midpoint_dynamics(obj,xin)
            ...
            x3 = xin(2+2*nX+(1:nX));
            u1 = xin(2+3*nX+(1:nU));
            u2 = xin(2+3*nX+nU+(1:nU));
            c2 = xin(2+3*nX+2*nU+(1:nC));
            b2 = xin(2+3*nX+2*nU+nC+(1:nC*nD));
            ...
            df = [df;
                  -vm1/h1, zeros(nQ,1), -1/h1*eye(nQ), zeros(nQ), 1/h1*eye(nQ), -eye(nQ), zeros(nQ,nX), zeros(nQ,2*nU), zeros(nQ,nC), zeros(nQ,nD*nC);
                  zeros(nQ,1), -vm2/h2, zeros(nQ,nX), -1/h2*eye(nQ), zeros(nQ), 1/h2*eye(nQ), -eye(nQ), zeros(nQ,2*nU), zeros(nQ,nC), zeros(nQ,nD*nC)];
        end
        
        function [f,df] = midpoint_contact_constraint_fun(obj,h,x2,psi,eta,c,b,s)
            
            xin = [h;x2;psi;eta;c;b;s];
...
%             disp(max(abs(df_fd(:)-df(:))));
        ...
        function [f,df] = midpoint_contact(obj,xin)
            ...
            x2 = xin(1+(1:nX));
            psi = xin(1+nX+(1:nC));
            eta = xin(1+nX+nC+(1:nD*nC));
            c = xin(1+nX+nC+nD*nC+(1:nC));
            b = xin(1+nX+2*nC+nD*nC+(1:nD*nC));
            s = xin(end);
            ...
            %xin = [h;x2;psi;eta;c;b;s];
            df = [zeros(nD*nC,1), kron(v2', eye(nD*nC))*dD, D, E', -eye(nD*nC), zeros(nD*nC,nC), zeros(nD*nC,nD*nC), zeros(nD*nC,1);
                  zeros(nC,1), n, zeros(nC,nQ), zeros(nC,nC), zeros(nC,nD*nC), zeros(nC,nC), zeros(nC,nD*nC), zeros(nC,1);
                  zeros(nC,1), zeros(nC,nQ), zeros(nC,nQ), zeros(nC,nD*nC), zeros(nC,nC), mu*eye(nC), -E, zeros(nC,1);
                  zeros(1,1), c'*n, zeros(1,nQ), zeros(1,nC), zeros(1,nD*nC), phi', zeros(1,nD*nC), -1;
        ...
        function [f,df] = midpoint_running_fun(obj,running_fun,h,x1,x2,u)
            xin = [h;x1;x2;u];
...
%             disp(max(abs(df_fd(:)-df(:))));
        ...
        function [f,df] = midpoint_running(obj,running_fun,xin)
            ...
            df = [dg(:,1), 0.5*dg(:,1+(1:nQ)), zeros(1,nQ), 0.5*dg(:,1+(1:nQ)), dg(:,1+nQ+(1:nQ)), dg(:,(1+nX+1):end)];
        end
        
        function [f,df] = midpoint_final_fun(obj,final_fun,h,xf)
            tf = sum(h);
            [f,dg] = final_fun(tf,xf);
            df = [kron(ones(1,obj.N-1),dg(:,1)), dg];
        end
        ...
        function xtraj = reconstructStateTrajectory(obj,z)
            ...
            switch obj.integration_method
                case VariationalTrajectoryOptimization.MIDPOINT
                    ...
                    v = [x(nQ+1:nQ,2:end), x(nQ+1:nQ,end)]; %zoh ignores the last entry in v
