classdef TimeSteppingRigidBodyManipulator < DrakeSystem
    ...
    % full version with constraint rho7
            ...
            for k = 1:nC
                ...
                for m = 1:MAX_ITER
                    ...
                    lambda_n_num(k) = rho(1)*(lambda_n_tilde(k) - d(1,k)) + rho(4)*mu(k)*(lambda_f_tilde(k) + OnesFull'*lambda_parallel_prev + d(4,k)) ...
                        - (- c_nc*g_nc_n_tilde + c_vc_multiply_g_vc_n_tilde_sum) + (1.0/t_nc + mC/t_vc)*lambda_n_prev ...
                        + lambda_n_num_sum;
                    ...
                    %                                       + lambda_n_num_sum;
                    lambda_n_den(k) = rho(1) + rho(4)*mu(k)^2 + 1.0/t_nc + mC/t_vc + lambda_n_den_sum;% rho(1) + rho(4)*mu(k)^2 + rho(6)*v_mag(k)^2*mu(k)^2 + 1.0/t_cc + 1.0/t_nc + mC/t_vc + lambda_n_den_sum;
                    ...
                    for i = 1:mC
                        ...
                        lambda_parallel_num(i) = - rho(4)*(lambda_f_tilde(k) - mu(k)*lambda_n_prev + OnesReduced'*lambda_parallel_prev_reduced + d(4,k)) ...
                            - (- c_nc*g_nc_t_tilde(i) + c_vc_multiply_g_vc_t_tilde_sum(i)) + (1.0/t_nc + mC/t_vc)*lambda_parallel_prev(i) + rho(10+i)*(lambda_parallel_tilde(i,k) - d(10+i,k)) + lambda_parallel_num_sum;
                        %                         lambda_parallel_num(i) = - rho(4)*(lambda_f_tilde(k) - mu(k)*lambda_n_prev + OnesReduced'*lambda_parallel_prev_reduced + d(4,k)) ...
                        ...
                        lambda_parallel_den(i) = rho(4) + rho(10+i) ...
                            + 1.0/t_nc + mC/t_vc + lambda_parallel_den_sum;
...
%                         end
                    ...
                    primal_residual = [(primal_var(:,k) - slack_var_selected(:,k)); lambda_f_tilde(k) - mu(k)*lambda_n(k) + OnesFull'*lambda_parallel(:,k); zeros(1,1); zeros(1,1); phi_tilde(k) - phi(k)];
                    ...
                    % dual residual
                    A = zeros(4 + mC*2, length(primal_var(:,k)));
                    B = zeros(4 + mC*2, length(slack_var(:,k)));
                    ...
                    %constant residual
                    c = [0,0,-w(k),-w(nC*(1+mC)+k),zeros(1,4),-w(nC+k),-w(nC*2+k),-w(nC*3+k),-w(nC*4+k)]';
