classdef KukaArmPlant_J6J7 < Manipulator 
  ...
  methods
    function obj = KukaArmPlant_J6J7
      ...
      obj = setInputFrame(obj,CoordinateFrame('KukaArmInput',2,'u',{'tau6','tau7'}));
      obj = setStateFrame(obj,CoordinateFrame('KukaArmState',4,'x',{'theta6','theta7', ...
                                                'theta6dot','theta7dot'}));
      obj = setOutputFrame(obj,obj.getStateFrame);
      
      obj.xG = Point(obj.getStateFrame,[0;0;0;0]);%[To Be Checked]
      obj.uG = Point(obj.getInputFrame,[0;0]);
...
%         {'bv1_positive','bv2_positive','bv3_positive','bv4_positive','bv5_positive','bv6_positive','bv7_positive','m1','m2','m3','m4','m5','m6','m7'}));
      obj = setParamFrame(obj,CoordinateFrame('KukaArmParams',2,'p',...
        {'I6zz','I7yy'})); %,'m2','m3','m4','m5','m6','m7'
...
%        { 'lc2' }));
    ...
    function [H,C,B] = manipulatorDynamics(obj,q,qd)
      ...
      c = cos(q(1:2,:));  s = sin(q(1:2,:));%[To be Checked]
      toc
      ...
      tic
      %H = getKukaArmInertiaMatrix_simplified(obj,q,c,s);
      h11 = I7yy + I6zz + (9*m6)/25000000 + (10201*m7)/1000000 + I7xx*sin(q7)^2 - I7yy*sin(q7)^2;
      h12 = 0;h21 = h12; 
      h22 = I7zz;
      H = [h11, h12;h21,h22];
      toc
      ...
      tic
      %C = getKukaArmCoriolisCentrifugalVector(obj,q,qd,c,s);
      C1 = 0;
      C2 = (dq6*dq7*sin(2*q6)*(I7xx - I7zz))/2;
      C = [C1;C2];
      toc
      %C = [ -2*m2l1lc2*s(2)*qd(2), -m2l1lc2*s(2)*qd(2); m2l1lc2*s(2)*qd(1), 0 ];
      %G = g*[ m1*lc1*s(1) + m2*(l1*s(1)+lc2*s12); m2*lc2*s12 ];
      tic
      %G = getKukaArmGravityVector(obj,q,c,s);
      G1 = -(g*sin(q6)*(3*m6 + 505*m7))/5000; 
      G2 = 0;
      G = [G1;G2];
      toc
...
%       end
      %b(1,1) = (bv6_positive + bv6_positive)/2 * qd(1) + bc6_positive;      
      %b(2,1) = (bv7_positive + bv7_positive)/2 * qd(2) + bc7_positive;
