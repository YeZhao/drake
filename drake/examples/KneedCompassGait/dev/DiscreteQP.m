classdef DiscreteQP < DrakeSystem
  ...
  function obj = DiscreteQP(r,com_ref,x_ref,options)
    ...
    if isfield(options,'Q_com')
      ...
      obj.Q_com = 1000*ones(4,1);% [Ye: modify to 2D compass gait walker case]
    end
...
%     bin = bin(bin~=inf);
    ...
    if nc > 0
      ...
      fqp = fqp - (obj.Q_com.*(obj.com_ref +[com;zeros(2,1)]))'*dc_next;% +[com;zeros(3,1)]))'*dc_next; [Ye: reduce one dimension for 2D compass walker]
