/// @file
///
/// kuka_inverse_dynamics_runner is designed to compute the torque command based on 
...
class InverseDynamicsRunner {
 public:
  /// tree is aliased
  explicit InverseDynamicsRunner(const RigidBodyTree<double>& tree)
      : tree_(tree), plan_number_(0), IDcontroller_trigger(false) {
    VerifyIiwaTree(tree);
    lcm_.subscribe(kLcmStatusChannel,
                    &InverseDynamicsRunner::HandleStatus, this);
    lcm_.subscribe(kLcmIDInputChannel,
                    &InverseDynamicsRunner::HandleInverseDynamicsInput, this);
  }
  ...
  void Run() {
    ...
    iiwa_inverse_dynamics_input_.utime = cur_time_us;
    ...
    while (true) {
      ...
      if (IDcontroller_trigger) {  
        ...
        for (int joint = 0; joint < kNumDof; joint++){
          joint_position_desired(joint) = iiwa_inverse_dynamics_input_.joint_position_desired[joint];
          joint_velocity_desired(joint) = iiwa_inverse_dynamics_input_.joint_velocity_desired[joint];
          joint_accel_desired(joint) = iiwa_inverse_dynamics_input_.joint_accel_desired[joint];
        }
 ...
 private:
  ...
  void HandleInverseDynamicsInput(const lcm::ReceiveBuffer* rbuf, const std::string& chan,
                    const lcmt_iiwa_inverse_dynamics_input* input) {
    iiwa_inverse_dynamics_input_ = *input;
    IDcontroller_trigger = true;
  }

  lcm::LCM lcm_;
  const RigidBodyTree<double>& tree_;
  int plan_number_{};
  bool IDcontroller_trigger;// inverse dynamics runner wait for the first message from plan runner 
  lcmt_iiwa_status iiwa_status_;
  lcmt_iiwa_inverse_dynamics_input iiwa_inverse_dynamics_input_;
};
...
int do_main(int argc, const char* argv[]) {
  ...
  InverseDynamicsRunner runner(tree);
  runner.Run();
  return 0;
}
