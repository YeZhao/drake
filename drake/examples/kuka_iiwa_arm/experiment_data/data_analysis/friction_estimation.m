...
friction_positive_set4 = [];
friction_negative_set4 = [];

friction_positive_set4 = load('FRICTION_POSITIVE_SET4.dat'); 
friction_negative_set4 = load('FRICTION_NEGATIVE_SET4.dat'); 

friction_positive_set4 = [friction_positive_set4;joint_vel_positive_avg,joint_torque_positive_avg];
friction_negative_set4 = [friction_negative_set4;joint_vel_negative_avg,joint_torque_negative_avg];

save FRICTION_POSITIVE_SET4.dat friction_positive_set4 -ASCII
save FRICTION_NEGATIVE_SET4.dat friction_negative_set4 -ASCII
...
figure(1)
coeffs_positive = polyfit(friction_positive_set4(:,1),friction_positive_set4(:,2), 1);
x1 = linspace(0, 1.0, 100);
y1 = polyval(coeffs_positive, x1);
coeffs_negative = polyfit(friction_negative_set4(:,1),friction_negative_set4(:,2), 1);
x2 = linspace(-1,0, 100);
...
hold on
plot(friction_positive_set4(:,1),friction_positive_set4(:,2),'ro','MarkerSize',10)
hold on
plot(friction_negative_set4(:,1),friction_negative_set4(:,2),'ro','MarkerSize',10)
xlabel('joint velocity [rad/s]','fontsize',mediumTextSize)
ylabel('joint torque [Nm]','fontsize',mediumTextSize)
title('Joint 4 friction model','fontsize',bigTextSize)
xlim([-1.2, 1.2])
ylim([-2, 2])
grid on
print -depsc Joint_4_friction_data_no_offset

figure(2)
% data with manual shifting
coeffs1 = polyfit(friction_positive_set4(:,1),friction_positive_set4(:,2), 1);
x1 = linspace(0, 1.0, 100);
...
ylim([-2, 2])
grid on
print -depsc Joint_4_friction_data
...
title('Joint 4 Torque V.S. Position')
...
ylabel('joint torque [Nm]')
title('Joint 4 Torque V.S. Velocity')
