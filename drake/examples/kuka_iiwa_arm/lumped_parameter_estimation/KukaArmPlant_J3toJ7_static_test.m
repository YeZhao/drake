classdef KukaArmPlant_J3toJ7_static_test < Manipulator 
  ...
  methods
    function obj = KukaArmPlant_J3toJ7_static_test
      obj = obj@Manipulator(5,5);%[To Be Checked]
      obj = setInputLimits(obj,-10,10);%[To Be Checked]

      obj = setInputFrame(obj,CoordinateFrame('KukaArmInput',5,'u',{'tau3','tau4','tau5','tau6','tau7'}));
      obj = setStateFrame(obj,CoordinateFrame('KukaArmState',10,'x',{'theta3','theta4','theta5','theta6','theta7', ...
                                                'theta3dot','theta4dot','theta5dot','theta6dot','theta7dot'}));
      obj = setOutputFrame(obj,obj.getStateFrame);
      
      obj.xG = Point(obj.getStateFrame,[pi/2;pi/2;pi/2;pi/2;pi/2;0;0;0;0;0]);%[To Be Checked]
...
%        { 'lc2' }));
    ...
    function [H,C,B] = manipulatorDynamics(obj,q,qd)
      ...
      tic
      c = cos(q(1:5,:));  s = sin(q(1:5,:));%[To be Checked]
      toc
      ...
      tic
      H = getKukaArmInertiaMatrix_J3toJ7(obj,q,c,s);
      toc
      
      tic
      C = getKukaArmCoriolisCentrifugalVector_J3toJ7(obj,q,qd,c,s);
      toc
      %C = [ -2*m2l1lc2*s(2)*qd(2), -m2l1lc2*s(2)*qd(2); m2l1lc2*s(2)*qd(1), 0 ];
      %G = g*[ m1*lc1*s(1) + m2*(l1*s(1)+lc2*s12); m2*lc2*s12 ];
      tic
      G = getKukaArmGravityVector_J3toJ7(obj,q,c,s);
      toc
